...
class ChatBubble: UIView {
    ...
    */
    ...
    var scrollViewWidth : CGFloat = 384
    /**
    ...
    */
    init(data: ChatBubbleData, startY: CGFloat){
        ...
        if let _ = data.text {
            ...
            labelChatText?.textColor = data.type == .Mine ? .white : .black
            labelChatText?.font = UIFont.systemFont(ofSize: 15)
        ...
        if data.type == .Mine {
            ...
            newStartX = scrollViewWidth - extraWidthToConsider - 10
        } else {
    ...
    class func framePrimary(type:BubbleDataType, startY: CGFloat) -> CGRect{
        let scrollViewWidth : CGFloat = 384
        let paddingFactor: CGFloat = 0.06
        let sidePadding = scrollViewWidth * paddingFactor
        let maxWidth = scrollViewWidth * 0.65 // We are cosidering 65% of the screen width as the Maximum with of a single bubble
        let startX: CGFloat = type == .Mine ? scrollViewWidth * (CGFloat(1.0) - paddingFactor) - maxWidth : sidePadding
